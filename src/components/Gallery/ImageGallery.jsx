...
const ImageGallery = forwardRef(({ filteredImages, title = "" }, ref) => {
  ...
  // Update data-index attributes for images on component mount
  useEffect(() => {
  ...
  return (
    <div ref={ref} className="px-4 py-8 min-h-screen mx-auto max-w-7xl">
          ...
          <ul ref={galleryRef} id="gallery" className="grid grid-cols-2 gap-5 lg:grid-cols-5">
            {filteredImages?.map((src, index) => (
              <li key={index} className="group relative pb-[75%] overflow-hidden rounded bg-gray-200 cursor-zoom-in">
                <img onClick={imageGalleryOpen} src={src} data-index={index + 1} className="absolute inset-0 object-cover w-full h-full group-hover:scale-105 transition-transform duration-500 ease-in-out" alt={`photo gallery image ${index + 1}`} />
              </li>
            ))}
          </ul>
        </div>
