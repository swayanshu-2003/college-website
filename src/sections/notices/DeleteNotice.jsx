...
const DeleteNotice = ({ setRefetchData, open, setOpen, id, showSnackBar, setSuccessStatus }) => {
    const [loading, setLoading] = useState(false)


    const handleRemoveNotice = async () => {
        setLoading(true);
        const deleteData = await axiosInstance.delete(`/notices/delete/${id}`);
        if (deleteData) {
            ...
            <div className='mx-auto w-full max-w-[550px] bg-white rounded-lg'>
                ...
                <div className='w-full px-4 py-2'>
                    <div className="w-full px-10 py-6">
                        ...
                        <LoadingButton onClick={handleRemoveNotice} loading={loading} variant="contained" color='error'>
                            Delete
                        </LoadingButton>
                    </div>
                </div>
