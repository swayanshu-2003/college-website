...
export default function UploadNotice({ isLoading, setIsloading, onNoticeUploaded, open, setOpen, showSnackBar, setSuccessStatus }) {
    ...
    const { setError, watch, getValues, handleSubmit, setValue, reset, formState: { errors } } = methods;
    ...
    const onSubmit = async (data) => {
        ...
        setIsloading(true);
        const uploadData = await axiosInstance.post(`/timetables/create`, formData, {
            ...
            <div className="flex items-center justify-center h-auto" onDragOver={handleDragOver} onDrop={handleDrop}>
                <div className="mx-auto w-[800px] bg-white rounded-lg">
                    <div className='w-full flex items-center justify-between bg-gray-800 rounded-t-lg px-4'>
                        <h1 className="w-auto text-white text-xl font-medium px-1 py-2">Publish TimeTable</h1>
                    ...
                    <FormProvider {...methods}>
                        <form className="py-4 px-9" onSubmit={handleSubmit(onSubmit)}>
                            ...
                            {errors?.link && (
                                <p className="text-red-500 mb-5 text-sm">{errors?.link?.message}</p>
                            )}
                            ...
                            <div className="w-full flex flex-row-reverse">
                                <button
                                    className="hover:shadow-form w-48 rounded-md bg-[#6A64F1] py-2 px-4 text-center text-base font-semibold text-white outline-none"
                                    type="submit"
                                >
                                    Publish TimeTable
                                </button>
